#!/home/nvidia/Downloads/envs/YOLO/bin/python3
# -*-coding: utf-8 -*-
...
class Yolov5Detector(yolov5.YOLOv5):
    ...
    def start_capture(self, video_file, save_video=None, detect_freq=1, vis=False):  #vis=False 
        """
        start capture video
    ...
    def draw_result(self, image: List[np.ndarray] or np.ndarray, dets, thickness=2, fontScale=0.8, delay=0, vis=False):
        """
    ...
    def draw_image(self, image, dets, thickness=1, fontScale=1.0, delay=0, vis=False):
        global pub
        global pub2
        content = {}
        ...
        print("cls", cls.tolist())
        # print(type(cls))
        # class_name:{0: 'red', 1: 'green', 2: 'yellow', 3: 'off'}
        content["cls"] = dets[:, 5].tolist()#from_latlon(38.99708624484, 117.29806014549)[0]
        content["conf"] = dets[:, 4:5].tolist()##from_latlon(38.99708624484, 117.29806014549)[1]
        content["boxes"] = dets[:, 0:4].tolist()##from_latlon(38.99708624484, 117.29806014549)[1]
        # print(type(content))
        # print(type(dets[:, 0:4]))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default="/home/nvidia/panda_ws/src/traffic_light/data/model/yolov5s_640/weights/best.pt", help='model.pt')
    parser.add_argument('--image_dir', type=str, default=None, help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--video_file', type=str, default=video_file, help='camera id or video file')
    parser.add_argument('--out_dir', type=str, default=None, help='save det result image')
...
if __name__ == "__main__":
    ...
    pub2 = rospy.Publisher("usb_cam/image_raw", Image, queue_size = 10) 
    ...
    if isinstance(opt.video_file, str):
        ...
        d.start_capture(opt.video_file, save_video, detect_freq=1, vis=False)
    else:
