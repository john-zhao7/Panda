...
def call_back_GNSS(msg):
    ...
    inc_x = 526007.0436536679
    inc_y = 4316258.079317236
    inc_point = [inc_x, inc_y]
    dis_light = math.sqrt(math.pow(ego_content['now_pos_x']-inc_x,2)+math.pow(ego_content['now_pos_y'] - inc_y,2))
    if previous_dis_light != None:
        diff = previous_dis_light - dis_light

    previous_dis_light = dis_light
    ...
    while not is_exit:
        ...
        if len(cls) != 0:
            ...
            if num_l >= 5:
                ...
                if cls[0] == 0:
                    #NOTE 刹车区间需要测试
                    if min_distance < 15:  # 红灯且5~10之间就刹车
                        Brake = True
                        if cls[0]== 1:
                            num_l = 0
                else:
...
def main():
    ...
    while not gps_flag: #  and not camera_flag :
        rospy.sleep(0.1)

    while not is_exit: #  and not camera_flag :
        start_time =  time.time()
        speed_from_plan = 20
        control_msg["PointVel"] = speed_from_plan #path_list_obj[mark].speed
        control_msg["target_speed"] = speed_from_plan#path_list_obj[mark].speed
        control_msg["deceleration"] = 0
        # print(dis_light, diff)
        if abs(diff) < 0.01:
            diff = 0
        if dis_light <= 15: 
            if abs(diff) < 0.01:
                diff = 0
            if diff < 0:
                if speed_from_plan <= dis_light:
                    control_msg["PointVel"] = dis_light #path_list_obj[mark].speed
                    control_msg["target_speed"] = dis_light
                    control_msg["deceleration"] = -0.8
                    if dis_light < 5:
                        control_msg["deceleration"] = -1
                ...
                control_msg["PointVel"] = 0
                control_msg["target_speed"] = 0
                control_msg["deceleration"] = -1
        print(dis_light, diff)
        wheelAngle = path_control.pathControl(CTE , diffCTE, dHead, diffdHead, yaml_data)
        control_msg["wheelAngle"] = wheelAngle
        print(wheelAngle, CTE, dHead)
        # control_msg["PointVel"] = 20 #path_list_obj[mark].speed
        # control_msg["CTE"] = CTE
        # control_msg["dHead"] = dHead
        # control_msg["Brake"] = Brake
        # control_msg["Brake_end"] = Brake_end
        ...
        # a = ego_content["now_speed"]
        msg = PathSpeedCtrlInterface()
        msg.Target_steering_angle = control_msg["deceleration"]
        msg.CTE = dis_light
        msg.dHead = diff
        msg.Target_velocity = control_msg["PointVel"]


        caculate_cte_wheel_angle()
