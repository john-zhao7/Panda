...
def readMap(mapfile):
    ...
    x_list = [pos.x for pos in lane.points]
    y_list = [pos.y for pos in lane.points]
    head_list = [pos.head for pos in lane.points]

    global_Map_dict = {
        "X_list" : x_list ,
        "Y_list" : y_list  ,
        "heading_list" : head_list
    }

    return global_Map_dict,x_list,y_list,head_list